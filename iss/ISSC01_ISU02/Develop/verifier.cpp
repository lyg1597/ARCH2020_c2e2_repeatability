...
int Verifier::verify()
{
    ...
    int refine_threshold = 10; // LMBTODO Move this to config file.
        ...
        if (trace_safe_flag == 0)
        {
            ...
            if (refine_method == 0)
            {
                ...
                else
                {
                    ...
                    std::cout << refine_idx << std::endl; // DEBUG
...
void Verifier::rewriteSimulationOutput(ReachTube &simulation_tube)
{
    ...
    if (sim_tube_size)
    {
        ...
        for (int i = 0; i < sim_tube_size; i++)
        {
            ...
            if (!guards_hit.empty())
            {
                simulation_tube.clear(i); // LMBTODO Config?
                break;
            }
        }
...
void Verifier::refine(RepPoint rep_point, int dim_index)
{
    ...
    for (int i = 1; i <= dim; i++)
    {
        p1.setCoordinate(i, max[i - 1] - delta_array[i - 1]);
    }
    rep2.setState(p2);
    ...
    if (numofvar > 0)
    {
        for (int i = 0; i < Simulation_data.rows(); i += CT_step)
        {
            ...
            for (int j = i; j < min(i + CT_step, (int)Simulation_data.rows()); j += 2)
            {
                ...
                std::cout<<CT_matrix.determinant()<<std::endl;
