...
extern "C" bool invariantSatisfied(int curMode, double *ptLower, double *ptUpper){
  ...
  if(curMode==1){
    ...
    if(ptLower[1]<ptUpper[1]){
      cs_box.insert(mult_factor*xp>=mult_factor*ptLower[1]);
    ...
    if(ptLower[2]<ptUpper[2]){
      cs_box.insert(mult_factor*yp>=mult_factor*ptLower[2]);
    ...
    if(ptLower[5]<ptUpper[5]){
      cs_box.insert(mult_factor*t>=mult_factor*ptLower[5]);
      cs_box.insert(mult_factor*t<=mult_factor*ptUpper[5]);
    }
    else{
      cs_box.insert(mult_factor*t<=mult_factor*ptLower[5]);
      cs_box.insert(mult_factor*t>=mult_factor*ptUpper[5]);
  ...
  if(curMode==3){
    ...
    box_poly = NNC_Polyhedron(cs_box);
    Pointset_Powerset<NNC_Polyhedron> box(box_poly);
    Pointset_Powerset<NNC_Polyhedron> invariant(1,UNIVERSE);
    Pointset_Powerset<NNC_Polyhedron> curInv;
    NNC_Polyhedron curPoly;
    Constraint_System cs;
    curInv = Pointset_Powerset<NNC_Polyhedron>(1,EMPTY);

    cs.set_space_dimension(1);
    cs.insert(10*t >= 2400);
    curPoly = NNC_Polyhedron(cs);
    curInv.add_disjunct(curPoly);
    cs.clear();
...
extern "C" vector<pair<NNC_Polyhedron, int> > hitsGuard(int curMode, double *ptLower, double *ptUpper){
  ...
  if(curMode==1){
    ...
    if(!guard.is_disjoint_from(box_poly)){
      ...
      if(ptLower[1]<ptUpper[1]){
        cs_box.insert(mult_factor*xp_temp>=mult_factor*ptLower[1]);
      ...
      if(ptLower[2]<ptUpper[2]){
        cs_box.insert(mult_factor*yp_temp>=mult_factor*ptLower[2]);
      ...
      if(ptLower[3]<ptUpper[3]){
        cs_box.insert(mult_factor*xd_temp>=mult_factor*ptLower[3]);
      ...
      if(ptLower[4]<ptUpper[4]){
        cs_box.insert(mult_factor*yd_temp>=mult_factor*ptLower[4]);
      ...
      if(ptLower[5]<ptUpper[5]){
        cs_box.insert(mult_factor*t_temp>=mult_factor*ptLower[5]);
      ...
      if(ptLower[6]<ptUpper[6]){
        cs_box.insert(mult_factor*loc_temp>=mult_factor*ptLower[6]);
      ...
      cs_gd.insert(10*yp_temp >= -1000);
      cs_gd.insert(10*xp_temp + 10*yp_temp >= -1411.0);
      cs_gd.insert(10*xp_temp >= -1000);
      cs_gd.insert(-10*xp_temp + 10*yp_temp <= 1411.0);
      cs_gd.insert(10*yp_temp <= 1000);
      cs_gd.insert(10*xp_temp + 10*yp_temp <= 1411.0);
      cs_gd.insert(10*xp_temp <= 1000);
  ...
  if(curMode==2){
    ...
    if(!guard.is_disjoint_from(box_poly)){
      ...
      vars.insert(loc_temp);
      guard_reset.remove_space_dimensions(vars);
      toRet.push_back(make_pair(guard_reset,3));
    }
  }
  if(curMode==1){
    Constraint_System cs_box;
