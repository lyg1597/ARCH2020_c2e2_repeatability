...
void Passive(const state_t &x, state_t &dxdt, const double t) {
			dxdt[0]=x[2];
			dxdt[1]=x[3];
			dxdt[2]=5.75894721132e-5*x[0] + 0.00876276*x[3];
			dxdt[3]=-0.00876276*x[2];
			dxdt[4]=1;
			dxdt[5]=0;
}

//ODE FUNCTION POINTER
void (*rhs[3])(const state_t &x, state_t &dxdt, const double t) =
	{ProxA, ProxB, Passive};
